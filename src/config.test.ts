...
import typedConfig, {NumberOptions} from './config'
...
describe('#getNumber', () => {
    ...
    context('when max value is provided and exceeded', () => {
        it('should throw with a useful error', () => {
            const env = {FOO: '9.1'}
            const opts: NumberOptions = {maxValue: 9}
            expect(() => typedConfig(env).getNumber('FOO', opts)).to.throw(Error, `Invalid configuration: value '9.1' for key 'FOO' is larger than max value of 9`)
        })
    })
    ...
    context('when min value is provided and the value is less than it', () => {
        it('should throw with a useful error', () => {
            const env = {FOO: '8.9'}
            const opts: NumberOptions = {minValue: 9}
            expect(() => typedConfig(env).getNumber('FOO', opts)).to.throw(Error, `Invalid configuration: value '8.9' for key 'FOO' is less than min value of 9`)
        })
    })
...
describe('#getBool', () => {
    ...
    context('when undefined and no default provided', () => {
        it('should throw with a useful error', () => {
            const env = {}
            expect(() => typedConfig(env).getBoolean('FOO')).to.throw(Error, `Invalid configuration: FOO must be 'true' or 'false' not undefined`)
        })
    })
    ...
    context('when defined, but not valid', () => {
        it('should return the default', () => {
            ...
            expect(() => typedConfig(env).getBoolean('FOO')).to.throw(Error, `Invalid configuration: FOO must be 'true' or 'false' not 'not_a_bool'`)
        })
    })
})